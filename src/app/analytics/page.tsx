...
async function safeGetOrgStats(): Promise<OrgStats> {
  try {
    ...
    const stats: OrgStats = {
      employees: raw.employees ?? raw.employeesCount ?? raw.employeeCount ?? 0,
      teams: raw.teams ?? raw.teamsCount ?? 0,
      openRoles: raw.openRoles ?? raw.openRolesCount ?? raw.jobsOpen ?? 0,
      eventsLogged: raw.eventsLogged ?? raw.events ?? raw.eventsCount ?? 0,
      newEvents24h: raw.newEvents24h ?? raw.newEventsLast24h ?? 0,
      activeTeams: raw.activeTeams ?? raw.teamsActive ?? 0,
    };
...
export default async function AnalyticsPage() {
  const [stats, events] = await Promise.all([safeGetOrgStats(), safeGetRecentEvents()]);
      ...
      <main className="px-8 py-8 space-y-8">
        ...
        <section className="grid gap-4 md:grid-cols-4">
          <div className="rounded-2xl border border-slate-800/70 bg-slate-950/70 p-4 shadow-sm">
            ...
            <p className="mt-1 text-[11px] text-slate-500">Synced via Intime HR API.</p>
          </div>
          ...
          <div className="rounded-2xl border border-slate-800/70 bg-slate-950/70 p-4 shadow-sm">
            ...
            <p className="mt-2 text-2xl font-semibold text-slate-50">{stats.teams}</p>
          ...
          <div className="rounded-2xl border border-slate-800/70 bg-slate-950/70 p-4 shadow-sm">
            ...
            <p className="mt-2 text-2xl font-semibold text-slate-50">{stats.openRoles}</p>
            <p className="mt-1 text-[11px] text-slate-500">Pulled from Jobs & ATS.</p>
          </div>

          <div className="rounded-2xl border border-slate-800/70 bg-slate-950/70 p-4 shadow-sm">
            <div className="flex items-center justify-between">
              <p className="text-xs font-medium text-slate-400">Events logged</p>
              <span className="rounded-full bg-slate-800 p-1">
                <Activity className="h-4 w-4 text-indigo-300" />
              </span>
            </div>
            <p className="mt-2 text-2xl font-semibold text-slate-50">{stats.eventsLogged}</p>
            <p className="mt-1 text-[11px] text-slate-500">
        ...
        <section className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-3 rounded-2xl border border-slate-800/70 bg-slate-950/70 p-5 shadow-sm">
              ...
              <ul className="divide-y divide-slate-800/80">
                {events.map((ev) => (
                  <li key={ev.id} className="flex items-start gap-3 py-3">
                    <div className="mt-1 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-indigo-500/15 text-xs font-medium text-indigo-200">
                      {ev.type?.[0] ?? "E"}
                    </div>
            ...
            <div className="flex items-center justify-end border-t border-slate-800/80 pt-3">
              <Link
                href={orgHref("/events?sort=newest")}
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border border-slate-800/70 bg-slate-950/70 p-4 shadow-sm">
              <div className="flex items-center justify-between">
                <h2 className="text-sm font-semibold text-slate-100">Org pulse</h2>
                <Clock className="h-4 w-4 text-slate-500" />
              </div>
              <ul className="mt-3 space-y-2 text-xs text-slate-300">
                <li>
                  <span className="font-medium text-slate-50">{stats.newEvents24h}</span>{" "}
                  new events (24h)
                </li>
                <li>
                  <span className="font-medium text-slate-50">{stats.eventsLogged}</span>{" "}
                  total tracked events
                </li>
                <li>
                  <span className="font-medium text-slate-50">{stats.activeTeams}</span>{" "}
            ...
            <div className="rounded-2xl border border-slate-800/70 bg-slate-950/70 p-4 shadow-sm">
              ...
              <ul className="mt-3 space-y-2 text-xs text-slate-300">
                <li>
                  Add activity via{" "}
                  <Link href={orgHref("/events/new")} className="text-indigo-300 hover:underline">
                    Add Event
                  </Link>
                  .
                </li>
                <li>
                  Invite teammates in{" "}
                  <Link href={orgHref("/teams")} className="text-indigo-300 hover:underline">
                    Teams
                  </Link>
                  .
                </li>
                <li>
                  Configure hiring in{" "}
                  <Link href={orgHref("/jobs")} className="text-indigo-300 hover:underline">
                    Jobs
                  </Link>
                  .
                </li>
            ...
            <div className="rounded-2xl border border-slate-800/70 bg-slate-950/70 p-4 text-[11px] text-slate-300">
              <p className="mb-1 font-medium text-slate-50">API health & debug</p>
              <p>
                Check your ingest pipeline and stats endpoints from the{" "}
                <Link href={orgHref("/health")} className="text-indigo-300 hover:underline">
                  API Health
                </Link>{" "}
                panel.
              </p>
