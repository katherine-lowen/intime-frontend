    ...
    async function fetchPipeline() {
      ...
      try {
        const res = await fetch(`${API_URL}/jobs/${jobId}/pipeline`, {
          headers: {
            // x-user-email / x-user-name can go here later if needed
          },
        });
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-500">
            Track candidates through each stage for this job. AI-scored
            applicants surface to the top so you can triage faster.
          </p>
        </div>
...
function StageColumn({
  ...
  return (
    <div className="flex w-[260px] flex-shrink-0 flex-col rounded-xl border border-slate-200 bg-white/90 p-3 shadow-sm">
      ...
      <div className="flex max-h-[480px] flex-1 flex-col gap-2 overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
...
function CandidateCard({
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      className="group flex flex-col rounded-lg border border-slate-100 bg-white px-3 py-2.5 text-left text-xs shadow-sm transition hover:-translate-y-0.5 hover:border-indigo-200 hover:bg-indigo-50/70 hover:shadow-md"
    >
