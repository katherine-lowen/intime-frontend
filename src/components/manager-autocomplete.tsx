...
export default function ManagerAutocomplete({ value, onChange, label = 'Manager', placeholder='Search name...' }: Props) {
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      if (!q) { setOpts([]); return; }
      try {
        const data = await api.get<Employee[]>(`/employees?q=${encodeURIComponent(q)}&limit=8`);
        setOpts(data);
        setOpen(true);
      } catch { /* noop */ }
    }, 200);
        ...
        <div className="mt-1 max-h-56 overflow-auto rounded border bg-white shadow">
          {opts.map(o => (
            <button
              ...
              onClick={() => { onChange(o.id); setQ(`${o.firstName} ${o.lastName}`); setOpen(false); }}
            >
              <span>{o.firstName} {o.lastName}</span>
              <span className="text-xs opacity-60">{o.department || o.title || ''}</span>
            </button>
          ))}
        </div>
      )}
